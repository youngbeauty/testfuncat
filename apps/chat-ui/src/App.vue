<template>
  <div class="flex flex-col h-screen bg-gray-100">
    ...
    <div
      class="bg-blue-600 p-1 flex items-center justify-between text-white shadow-md"
    >
      <button
        @click="deployAIChat"
      ...
      <div>
        <select
          id="model-select"
          v-model="model"
          class="select p-1 py-2 rounded bg-white text-black shadow-sm"
        >
    ...
    <!-- Center Column (Chat) -->
    <div
      class="flex-grow flex flex-col w-full bg-gray-900 text-white p-4 overflow-auto"
    >
      <div class="flex-grow overflow-y-auto mb-4">
        <ul class="space-y-4">
          <li
            ...
            :key="index"
            class="bg-gray-800 p-3 rounded shadow-sm"
          >
...
export default {
  ...
  methods: {
    deployAIChat() {
      window.open(
        "https://functioncat.cn/console/clone-system?sourceOrgSlug=template&sourceSystemSlug=aichat3",
        "_blank"
      );
    },
    async sendMessage() {
